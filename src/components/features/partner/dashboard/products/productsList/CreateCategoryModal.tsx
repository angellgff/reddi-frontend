...
interface CreateCategoryModalProps {
  isOpen: boolean;
  onClose: () => void;
  // Callback opcional si queremos actualizar el estado local sin recargar u optimistically
  ...
  return (
    <Portal>
      ...
      >
        ...
        >
          <h2 id="create-category-title" className="text-xl font-semibold mb-4 text-gray-800">
            Crear nueva categoría
          </h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <BasicInput
              id="new-category-name-products"
              label="Nombre"
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                type="submit"
                className="px-5 py-2 rounded-lg bg-primary text-white text-sm font-medium disabled:opacity-60 flex items-center gap-2 hover:bg-green-700 transition-colors"
                disabled={loading}
              >
                {loading && <Spinner className="h-4 w-4 border-white" />} Crear
              </button>
            </div>
          </form>
        </div>
