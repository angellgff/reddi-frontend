...
export default function ProductImportModal() {
  ...
  const handleDownloadTemplate = async () => {
    try {
      // @ts-ignore
      const { getImportTemplateAction } = await import("@/src/lib/partner/bulkUploadActions");
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-50 w-full max-w-lg translate-x-[-50%] translate-y-[-50%] rounded-xl bg-white p-6 shadow-lg focus:outline-none">
          ...
          <Dialog.Description className="text-sm text-gray-500 mb-4 flex flex-col gap-2">
            <span>Carga un archivo Excel (.xlsx) con tus productos. Asegúrate de seguir el formato correcto.
            Las columnas requeridas son: <strong>Name, Description, Price, Unit, Category</strong>.</span>
            <button 
          ...
          <div className="space-y-4">
            ...
            {result && (
                <div className={cn("p-4 rounded-lg text-sm", result.success ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700")}>
