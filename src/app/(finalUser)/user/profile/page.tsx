...
export default async function ProfilePage() {
  ...
  const [{ user: profile }, addressesRes, paymentsRes] = await Promise.all([
    getUserProfile(),
    getUserAddressesAndSelected(),
    (async () => {
      const {
        data: { user },
  ...
  // Auth email (source of truth) for display
  const {
    data: { user: authUser },
  } = await supabase.auth.getUser();
  ...
  return (
    <>
      <div className="hidden md:block mx-auto w-full max-w-6xl px-4 sm:px-6 md:px-8 py-6 md:py-8">
        {/* Header */}
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  ...
                  <Link
                    ...
                    className="h-10 px-4 rounded-xl border border-neutral-800 text-neutral-900 text-sm inline-flex items-center"
                  >
                    Editar
                  </Link>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  />
                  <InfoBlock label="Fecha de nacimiento" value={"—"} />
                </div>
              </div>
            </CardContent>
          </Card>
...
function InfoBlock({ label, value }: { label: string; value: string }) {
  return (
    <div className="space-y-1">
      ...
      <div className="text-sm md:text-base text-neutral-900">{value}</div>
    </div>
  );
}
